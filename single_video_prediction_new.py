...
def add_column_to_csv(input_file, output_file, column_name, column_data):
    with open(input_file, 'r') as input_csv, open(output_file, 'w', newline='') as output_csv:
        ...
        # Iterate over the rows and add the new column data
        for row in reader:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prop_generator_model_path',default='/home/intern/interndata/vipul/BMT/sample/best_prop_model.pt', required=False)
    parser.add_argument('--pretrained_cap_model_path',default='/home/intern/interndata/vipul/BMT/sample/best_cap_model.pt', required=False)
 #   parser.add_argument('--vggish_features_path', required=False)
  #  parser.add_argument('--rgb_features_path', required=True)
   # parser.add_argument('--flow_features_path', required=True)
    parser.add_argument('--duration_in_secs',default=500,type=float, required=False)
    parser.add_argument('--device_id', type=int, default=0)
    parser.add_argument('--max_prop_per_vid', type=int, default=100)
    parser.add_argument('--nms_tiou_thresh', default=0.4, type=float, help='removed if tiou > nms_tiou_thresh. In (0, 1)')
    args = parser.parse_args()

    base_path='/home/intern/interndata/vipul/BMT/test/vid_fr'
    captions_list=[]
    for number in range(100):
        flow_path = f"{base_path}/{number}_flow.npy"
        vggish_path = f"{base_path}/{number}_vggish.npy"
        rgb_path = f"{base_path}/{number}_rgb.npy"
    ...
    with open(output_file, 'w', newline='') as file:
        ...
        writer.writerow(zip(captions_list))
